...
function initAdminRuntimeSystems(deps = {}) {
  ...
  const RESET_DATA_UTILS = createResetDataUtils({
    ...
    coreDeletePlayerStorage: (id) => CORE.deletePlayerStorage(id),
    coreClearAllPlayerStorage: () => CORE.clearAllPlayerStorage(),
    coreResetWorldState: (payload) => CORE.resetWorldState(payload),
    walletDeleteWallet: (id) => WALLET.deleteWalletBinding(id),
    walletReplaceAll: (settings) => WALLET.replaceWalletSettings(settings),
    scratchResetState: () => ECON.resetScratchState(),
