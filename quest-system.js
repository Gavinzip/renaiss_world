...
const QUESTS = {
  ...
  新手任務: {
    title: "📜 新手任務",
    tasks: [
      { id: "explore_town", desc: "探索襄陽城", reward: "💰 50 Rns", target: 1 },
      { id: "learn_skill", desc: "學習一招戰技", reward: "⚔️ 戰力+3", target: 1 },
      { id: "make_friend", desc: "認識一位探索者", reward: "🤝 聲望+10", target: 1 },
  ...
  歷練任務: {
    title: "⚔️ 區域歷練",
    tasks: [
      { id: "defeat_bandit", desc: "擊敗山匪嘍囉", reward: "💰 100 Rns", target: 1 },
      { id: "visit_sect", desc: "拜訪任一陣營", reward: "📍 解鎖陣營", target: 1 },
  ...
  探索者之路: {
    title: "🗡️ 探索者之路",
    tasks: [
      { id: "defeat_boss", desc: "挑戰首領成功", reward: "🔥 技術檔案", target: 1 },
      { id: "find_secret", desc: "發現一個秘境", reward: "💎 珍稀物品", target: 1 },
      { id: "recruit_hero", desc: "招募一位同伴", reward: "👥 探索者加入", target: 1 },
...
function givePlayerQuest(player) {
  ...
  if (level <= 2) {
    ...
    questPool = [...QUESTS.新手任務.tasks, ...QUESTS.歷練任務.tasks, ...QUESTS.探索者之路.tasks];
  }
...
function getSuggestion(player) {
  ...
  if ((player.stats?.財富 || 50) < 20) {
    suggestions.push("💰 Rns 不多了！去集市打工或接受委託");
  }
  
  if (!player.quests || Object.keys(player.quests).length === 0) {
    suggestions.push("📜 你還沒有任務！去情報酒吧打聽消息");
  }
  ...
  return suggestions.slice(0, 3).map((s) => sanitizeWorldText(s));
}
...
function generateDailyEvent(player) {
  ...
  if (rand < 0.2) {
    ...
    events.push("👤 一位神秘的探索者路過，多看了你一眼...");
  } else if (rand < 0.6) {
    events.push("📦 路邊似乎有什麼閃光的東西...");
  }
  
  return sanitizeWorldText(events[Math.floor(Math.random() * events.length)]);
}
